...
class LazySupervisedDataset(Dataset):
    ...
    def collate_fn(self, batch):
        ...
        audio_labels_batch, text_labels_batch, audio_loss_mask_batch, text_loss_mask_batch = [], [], [], []
        whisper_features = []

        # Process each sample in the batch
        for sample in batch:
            # Pad and process audio-related tensors
            audio_pad_len = max_input_ids_len - sample['input_ids'].shape[1]
            input_ids_batch.append(torch.nn.functional.pad(sample['input_ids'], (0, audio_pad_len), value=self.pad_token).squeeze(0))
            is_continuous_mask_batch.append(torch.nn.functional.pad(sample['is_continuous_mask'], (0, audio_pad_len), value=False).squeeze(0))
            
            audio_labels, text_labels, audio_loss_mask, text_loss_mask = sample['labels']
            # text_labels,  text_loss_mask = sample['labels']
            audio_labels_batch.append(torch.nn.functional.pad(audio_labels, (0, audio_pad_len), value=self.pad_token).squeeze(0))
            audio_loss_mask_batch.append(torch.nn.functional.pad(audio_loss_mask, (0, audio_pad_len), value=False).squeeze(0))

            # Pad and process text-related tensors
            text_pad_len = max_input_ids_len - sample['text_input_ids'].shape[1]
            text_input_ids_batch.append(torch.nn.functional.pad(sample['text_input_ids'], (0, text_pad_len), value=self.pad_token).squeeze(0))
            text_labels_batch.append(torch.nn.functional.pad(text_labels, (0, text_pad_len), value=self.pad_token).squeeze(0))
            text_loss_mask_batch.append(torch.nn.functional.pad(text_loss_mask, (0, text_pad_len), value=False).squeeze(0))

            # Assuming one audio feature per sample
            if sample['whisper_input_feature']:
                whisper_features.append(sample['whisper_input_feature'][0])
        ...
        return dict(
            ...
            labels=(
                torch.stack(audio_labels_batch),
                torch.stack(text_labels_batch),
                torch.stack(audio_loss_mask_batch),
