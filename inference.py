...
def infer(model, prompt_manager, audio_path, text, prompt):
    ...
    with torch.no_grad():
        ...
        sent_logits, word_logits = outputs.logits
        
        # Sentence
        sent_probs = torch.softmax(sent_logits, dim=-1)
        sent_pred = torch.argmax(sent_probs, dim=-1).item()
        sent_probs_list = sent_probs[0].tolist()
        
        # Word
        word_probs = torch.softmax(word_logits, dim=-1)
        word_preds = torch.argmax(word_probs, dim=-1)
        word_probs_list = word_probs[0].tolist()
        word_preds_list = word_preds[0].tolist()

    actual_word_count = len(text.strip().split()) 

    word_preds_list = word_preds_list[:actual_word_count]
    word_probs_list = word_probs_list[:actual_word_count]

    return sent_pred, sent_probs_list, word_preds_list, word_probs_list

def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--model_path", type=str, default='/mnt/pfs_l2/jieti_team/SFT/hupeng/resources/ts_model/wt6s_snt_word_acc_model_v3.1', help="Path to the trained model checkpoint")
    ...
    for dataset_name, file, wavpath, prompt in infer_data:
        ouput_file = os.path.join(args.model_path, 'infer_res', f"{dataset_name}.res")
        ...
        with open(file, 'r') as fin:
            ...
            for line in tqdm(lines):
                try:
                    ...
                    sent_pred, sent_probs, word_preds, word_probs = infer(model, prompt_manager, audio, text, prompt)
                    end_time = time.time()
...
def ad_main():
    ...
    for data in tqdm(infer_data, total=len(infer_data)):
        ...
        with torch.no_grad():
            ...
            sent_logits, _ = outputs.logits
            probabilities = torch.softmax(sent_logits, dim=-1)
